...
<body class="gray-bg">
					...
					<form id="form_user" method="post" action="reserveUser.htm">
						...
						<table style="border-collapse:separate; border-spacing:0px 10px;">
							...
							<tr>
								<td>爱好：</td>
								<td>
									<input type="checkbox" id="hobby1" name="hobby" value="吃" class="" aria-required="true" required/>吃
									<input type="checkbox" id="hobby2" name="hobby" value="喝" class="" aria-required="true" required/>喝
									<input type="checkbox" id="hobby3" name="hobby" value="玩" class="" aria-required="true" required/>玩
								</td>
								<td>&nbsp;&nbsp;</td>
								<td>生日：</td>
	...
	<!-- 添加附表对话框 -->
	<div class="modal fade" id="modal_t_edit" role="dialog" aria-labelledby="modal_t_edit" aria-hidden="true">
		<div class="modal-dialog">
			<div class="modal-content">
				<div class="modal-body">
					<form id="form_t" method="post" action="reservet.htm">
						<table style="border-collapse:separate; border-spacing:0px 10px;">
							<tr>
								<td>老师姓名：</td>
								<td>
									<input type="text" id="tname1" name="tname" placeholder="输入新老师记录" class="" aria-required="true" required/>
								</td>
							</tr>
						</table>
						
						<div class="modal-footer">
							<button type="button" class="btn btn-primary"  id="submit_form_t_btn">保存</button>
							<button type="button" class="btn btn-default" data-dismiss="modal">关闭</button>
						</div>
					</form>
	...
	$(function () {
	    ...
	    $("#submit_form_t_btn").click(function(){
	    	$("#form_t").submit();
	    });
	});
	...
	var ButtonInit = function () {
	    ...
	    oInit.Init = function () {
	    	...
	    	$("#btn_addt").click(function(){
	    		$("#form_t").resetForm();
	    		$('#modal_t_edit').modal({backdrop: 'static', keyboard: false});
				$('#modal_t_edit').modal('show');
	        });
	    	...
	    	$("#btn_c").click(function(){
	    		$.ajax({
	    		    ...
	    		    success:function(res){
	    		    	if(res.success){
							var abc = echarts.init(document.getElementById("main"));
							var xdata = [];
							var ydata = [];
							res.data.forEach(function(f) {
								xdata.push(f.teachername);
								ydata.push({value : f.num,	name : f.teachername});
							});
							var	option = {
							    title: {
							        text: '某站点用户访问来源',
